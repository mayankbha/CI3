...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $additional_data = array(), $groups = array(), $registered_by='',$actv=0)
	{
		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('manual_activation', 'ion_auth');

		if ($this->identity_check($identity) && $registered_by!='main')
		{
		...
		if($registered_by=="googleplus" || $registered_by=="facebook" || $registered_by="main")
			$actv=1;
...
//var_dump($this->ion_auth->errors());die();
	...
	**/
	...
	public function login_via_main($identity, $password, $remember=FALSE, $fb=FALSE,$group='')
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select('*')
		                  ->where($this->identity_column, $identity)
		                  ->where('user_belongs_group',$group)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);	
