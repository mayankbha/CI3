...
class Base_Model extends CI_Model  
{
	...
	function get_page_privacy_policy(){

		$pagePrivachAndPolicy = $this->db->get_where($this->db->dbprefix('pages'), array('id' => '4'))->result();
		return $pagePrivachAndPolicy;
	}

	function get_page_cookies_policy(){

		$pageCookiesAndPolicy = $this->db->get_where($this->db->dbprefix('pages'), array('id' => '1'))->result();
		return $pageCookiesAndPolicy;
	}

	function get_page_refund_policy(){

		$refund_policy = $this->db->get_where($this->db->dbprefix('pages'), array('id' => '5'))->result();
		return $refund_policy;
	}	

	function get_page_disclaimer(){

		$disclaimer = $this->db->get_where($this->db->dbprefix('pages'), array('id' => '6'))->result();
		return $disclaimer;
	}	
	...
	function get_usersCount(){
		...
		$query ="SELECT COUNT( * ) AS total_users, COUNT( IF( ug.group_id =2, 1, NULL ) ) AS total_buyers, COUNT( IF( ug.group_id =3, 1, NULL ) ) AS total_sellers, COUNT( IF( ug.group_id =4, 1, NULL ) ) AS total_institutes FROM " .$this->db->dbprefix('users'). " u  INNER JOIN ". $this->db->dbprefix('users_groups') ." ug ON ug.user_id = u.id WHERE u.id !=1";
	...
	function get_packages_subscriptions()
	{
		...
		$query = "SELECT
					...
					(select count(*) from pre_subscriptions where user_type = 'Buyer' AND package_id =p.id) as Buyers,
					(select count(*) from pre_subscriptions where user_type = 'Seller' AND package_id =p.id) as Sellers,
