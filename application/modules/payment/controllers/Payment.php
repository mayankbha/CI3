	...
	function index()
	{
		if(isset($_POST['Submit']))
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				if(count($gateway_details) > 0 && count($package_info) > 0)
				{
					...
					elseif($gateway_details[0]->type_id == PAYU_PAYMENT_GATEWAY ) //Payu Settings
					{
						...
						$url = base_url() . 'buyer/paywith_razorpay/package/'.$package_id.'/gateway/'.RAZORPAY_PAYMENT_GATEWAY;
						redirect( $url );
					}elseif ( $gateway_details[0]->type_id == MANUAL_TRANSFER ) {

						// echo "manual ".'<br>';

						$this->insert_transaction( 'manual', array('is_valid_request_clear' => 'yes', 'is_redirect' => 'no') );

						$this->prepare_flashmessage(get_languageword('Payment Request Successful'), 0);
						$user_type = getUserType();
						if($user_type == "buyer")
							redirect(URL_BUYER_SUBSCRIPTIONS, 'refresh');
						else if($user_type == "seller")
							redirect(URL_SELLER_SUBSCRIPTIONS, 'refresh');
			...
			else
			{
				...
				$user_type = getUserType();
				if($user_type == "buyer")
					$redirect_path = URL_BUYER_LIST_PACKAGES;
				else if($user_type == "seller")
					$redirect_path = URL_SELLER_LIST_PACKAGES;
		...
		if ( ! empty( $_POST ) ) {
			...
			if ( ! empty( $_POST['razorpay_payment_id'] ) ) {
				...
				if( $response['status'] == 'success' ) {
					...
					$this->safe_redirect(site_url('buyer/list_packages'), 'Payment failed. Please try again');
				}
