...
class Auth extends MY_Controller {
	...
	function index($param = '', $param2 = '')
	{
		...
		{
			//Allow Institutes to add their Sellers
		...
		else
		{
			...
			// $user_group_arr = array('2' => 'Buyer', '3' => 'Seller', '4' => 'Institute');

			$user_group_arr = array('1' => 'Admin', '2' => 'Buyer', '3' => 'Seller', '4' => 'Institute');
			...
			if ($param == '3') {
				...
				if($crud_state == 'list'){
					echo '<style>.read-icon{display: none!important;}</style>'; 
				}
				$crud->columns('id','email', 'first_name', 'last_name', 'paypal_email','admin_commission', 'active');
				$crud->edit_fields('active','admin_commission');
			
			}elseif ($param == '2') {

				if($crud_state == 'list'){
				// 	echo '<style>.edit-icon{display: none!important;}.read-icon{display: none!important;}</style>';
				echo '<style>.read-icon{display: none!important;}</style>';  
				}
				$crud->columns('id','email', 'first_name', 'last_name', 'paypal_email', 'admin_discount','admin_approved', 'active');
				$crud->edit_fields('active', 'admin_discount', 'force_discount');
			
			}else {

				$crud->columns('id','email', 'first_name', 'last_name', 'active');

				$crud->add_fields('email','first_name','last_name', 'gender', 'phone_code', 'phone', 'password', 'confirm_password');

				$crud->edit_fields('first_name','last_name', 'gender', 'active', 'phone_code', 'phone', 'email');
			}
			
			$crud->set_field_upload('photo','assets/uploads/profiles');
			$crud->required_fields(array('username', 'password', 'email', 'first_name', 'gender', 'phone_code', 'phone', 'confirm_password', 'admin_discount','admin_commission'));
			...
			$crud->display_as('admin_commission', get_languageword('Admin_Commission_(%)'));
			$crud->display_as('admin_discount', get_languageword('Admin_Discount_(%)'));
			...
			//Custom action button for approving Seller|Institute
			if(($param == 3 || $param == 4) && $this->ion_auth->is_admin()) {
				$crud->callback_column('admin_approved', array($this,'_call_back_set_action'));
			}

			//No Admin approvement required for Buyers
			if(empty($param) || $param == 2 || (strcasecmp(get_system_settings('need_admin_for_seller'), 'no') == 0) || $this->ion_auth->is_institute()) {
			...
			$crud->field_type('force_discount','dropdown', [
				'0' => 'Keep maximum discount available',
				'1' => 'Force admin discount'
			]);
			...
			if($this->ion_auth->is_admin()){
				if($param==2 || $param==3 || $param==4)
					$crud->unset_add();
				if($param == 4){
					$crud->add_action(get_languageword('view_institute_sellers'), URL_ADMIN_IMAGES.'users.png',  URL_ADMIN_VIEW_INST_SELLERS.'/');
				}
			...
			if($crud_state != 'list')
			{
				if(!empty($param))
				{
					...
					if($param == 2)
					{
						$title = get_languageword('buyers');
						$crud->set_subject(get_languageword('buyer'));
					}
					if($param == 3)
					{
						$title = get_languageword('sellers');
						$crud->set_subject(get_languageword('seller'));
					}
					if($param == 4)
					{
			...
			if($this->ion_auth->is_institute()) {
				...
				$this->data['pagetitle'] = get_languageword('Manage Sellers');
			}
	...
	function encrypt_password_and_update_callback( $post_array, $primary_key )
	{
		...
		$additional_data = array(
								...
								'user_belongs_group' 	=> $post_array['user_belongs_group'],
								'admin_commission'      => (isset($post_array['admin_commission']) && $post_array['admin_commission'] != '') ? (float)$post_array['admin_commission'] : null,
								'admin_discount'      => (isset($post_array['admin_discount']) && $post_array['admin_discount'] != '') ? (float)$post_array['admin_discount'] : null,
								'force_discount'      => (isset($post_array['force_discount']) && $post_array['force_discount'] != '') ? $post_array['force_discount'] : '0',
								'last_updated' 			=> date('Y-m-d H:i:s'),
							);
		
		if(empty(trim($additional_data['username']))) {
			unset($additional_data['username']);
		}
		if(empty($additional_data['first_name'])) {
			unset($additional_data['first_name']);
		}
		if(empty($additional_data['last_name'])) {
			unset($additional_data['last_name']);
		}
		if(empty($additional_data['phone_code'])) {
			unset($additional_data['phone_code']);
		}
		if(empty($additional_data['country'])) {
			unset($additional_data['country']);
		}
		if(empty($additional_data['phone'])) {
			unset($additional_data['phone']);
		}
		if(empty($additional_data['gender'])) {
			unset($additional_data['gender']);
		}
		if(empty($additional_data['user_belongs_group'])) {
			unset($additional_data['user_belongs_group']);
		}

		$prev_username = $this->base_model->fetch_value('users', 'username', array('id' => $primary_key));
		$prev_group = $this->base_model->fetch_value('users', 'user_belongs_group', array('id' => $primary_key));
		...
		if(empty($post_array['user_belongs_group'])) {
			$post_array['user_belongs_group'] = $prev_group;
		}
			...
			if ($this->form_validation->run() == true)
			{
				...
				if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
				{
					...
					if($this->session->userdata('req_from')) {
						...
						if($this->session->userdata('req_from') == "leads" && ($this->ion_auth->is_seller() || $this->ion_auth->is_institute()) )
							redirect(URL_HOME_SEARCH_BUYER_LEADS);
						else if($this->session->userdata('req_from') == "buy_book")
							redirect(URL_HOME_BUY_BOOK.'/'.$this->session->userdata('selling_book_slug'));
					}

					if(!$this->ion_auth->is_admin() && !is_inst_seller($user_id)) {
					...
					if($this->ion_auth->is_admin()) {
						...
						if(!is_inst_seller($user_id) && $is_profile_updated != 1) {

							$this->prepare_flashmessage(get_languageword('please_update_your_profile_by_adding_sellering_books_and_preferred_teaching_types_to_avail_for_buyers'), 2);
							redirect(URL_SELLER_MANAGE_BOOKS, 'refresh');
						}
						...
						if($is_profile_updated != 1) {
							...
							$this->prepare_flashmessage(get_languageword('please_update_your_profile_by_adding_preferred_books_and_preferred_teaching_types_to_get_sellers'), 2);
							redirect(URL_BUYER_MANAGE_BOOKS, 'refresh');
						}

						redirect(URL_BUYER_INDEX, 'refresh');
					}
						...
						if($is_profile_updated != 1) {
							...
							redirect(URL_INSTITUTE_OFFERED_BOOKS, 'refresh');
						}
		...
		if(isset($_POST['create']))
		{
			...
			if ($this->form_validation->run() == true)
			{
				...
				if ($id)
				{
					...
					$type = get_languageword('buyer');
					$content=get_languageword("new_buyer_registered");
					$link = SITEURL.'auth/index/2/read/'.$id;
					if ($user_belongs_group==3) {
					    $type = get_languageword('seller');
						$content=get_languageword("new_seller_registered");
...
//var_dump($this->form_validation->run());die();
	...
	*/
	...
	function google_openid()
	{
	  ...
	  //function login via main site
    public function login_via_main()
    {
    	
    	// echo "<pre>";print_r($this->input->post());
     //    die();

        $this->is_user_exist($this->input->post());

        // $this->register_user_via_main($this->input->post());
    ...
    public function is_user_exist($data)
    {
        ...
        $password = $this->ion_auth->hash_password_db('',$data['password']);

        $credential = array(
        	'email' => $data['email'],
        	'password'=>$password,
        	'user_belongs_group'=>$data['group']
        );

        // echo "<pre>";print_r($data);
        // echo "<pre>";print_r($credential);
        // die();

    /*Jk working*/
    // Checking login credential for admin
    $query = $this->db->get_where('pre_users', $credential);
    ...
    //     die();
	    ...
	    if ($query->num_rows() > 0) {
	    	...
	    	if($row)
	    	{
	    		...
	    		$this->db->where('id',$row->id)->update('pre_users',array('auth_key'=>$data['auth_key']));

	    		echo base_url('/auth/login_via_authkey/'.$data['auth_key']);
	    	...
	    	$this->register_user_via_main($data);
	    }
	}

    // register user via main site
    public function register_user_via_main($input) {
      ...
      //   die();
            ...
            $email  =html_escape($input['email']) ;
				...
				$additional_data = array(
					...
					'first_name' 			=>html_escape($input['fname']),
					'last_name'  			=> html_escape($input['lname']),
					'current_language'      => 'english',

					'normal_password'=>$input['password'],
					// 'phone_code'     	=> $this->input->post('phone_code'),
					'user_belongs_group'=>$input['group'],
				...
				/* if ($this->form_validation->run() == true && $this->ion_auth->register($identity, $password, $email, $additional_data,$group)) */
				$user_id = $this->ion_auth->register($identity, $password, $email, $additional_data,$group,'main');

                  if($user_id)
                  {
                    $this->db->where('id',$user_id)->update('pre_users',array('auth_key'=>$input['auth_key']));
                  }


               echo base_url('/auth/login_via_authkey/'.$input['auth_key']);

    }


    public function login_via_authkey(string $authKey)
    {

    	// echo "<pre>";
    	// $row=$this->db->where('auth_key',$authKey)->get('pre_users')->row();
     //    print_r($row);
     //    die();

       if($authKey)
       {
          if($row=$this->db->where('auth_key',$authKey)->get('pre_users')->row())
          {
          ...
          $this->db->where('id',$row->id)->update('pre_users',array('auth_key'=>''));

      $this->ion_auth->login_via_main($row->email, $row->normal_password, false,false,$row->user_belongs_group);
     ...
     // echo '<pre>';print_r($this->session->userdata());die;
					...
					if($this->ion_auth->is_admin()) {
						...
						if(!is_inst_seller($user_id) && $is_profile_updated != 1) {
							...
							$this->prepare_flashmessage(get_languageword('please_update_your_profile_by_adding_sellering_books_and_preferred_teaching_types_to_avail_for_buyers'), 2);
							redirect(URL_SELLER_PROFILE, 'refresh');
						}
						...
						if($is_profile_updated != 1) {
							...
							$this->prepare_flashmessage(get_languageword('please_update_your_profile_by_adding_preferred_books_and_preferred_teaching_types_to_get_sellers'), 2);
							redirect(URL_BUYER_PROFILE, 'refresh');
						}
