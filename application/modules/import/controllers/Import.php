	...
	function index( $type, $menu = '' )
	{
		...
		if( isset( $_POST['submitbutt'] ) ) {
			...
			if($this->form_validation->run()!=false)
			{
				...
				} elseif( $type == 'books' && $this->ion_auth->is_admin() ) {
							...
							if( $type == 'users' ) {
								foreach($result as $row)
								{
									...
									if( $this->is_duplicate('users', array( 'email' => $email )) == TRUE ) {
										...
										$group = ( in_array($group, array(1,2,3,4)) ) ? $group : 2;
										$inputdata['user_belongs_group'] = $group;
										$groups = array( $group );			
							...
							elseif( $type == 'categories' ) {
								foreach($result as $row)
								{
									...
									if( ( $this->is_duplicate('categories', array( 'name' => $name )) == TRUE ) || ( $this->is_duplicate('categories', array( 'code' => $code )) == TRUE ) ) {
										...
										$inputdata['is_parent'] = 1; // 1 - Category, 0 - Book
										$inputdata['status'] = 1;
										$inputdata['sort_order'] = isset($row['sort_order']) ? $row['sort_order'] : '';
										$inputdata['created_by'] = isset($row['created_by']) ? $row['created_by'] : 1;
							...
							elseif( $type == 'books' ) {
								foreach($result as $row)
								{
									...
									if( ( $this->is_duplicate('categories', array( 'name' => $name )) == TRUE ) || ( $this->is_duplicate('categories', array( 'code' => $code )) == TRUE ) ) {
										...
										$inputdata['code'] = $code;
										$inputdata['description'] = isset($row['description']) ? $row['description'] : '';
										$inputdata['is_parent'] = 0; // 1 - Category, 0 - Book
										$inputdata['status'] = 1;
										$inputdata['sort_order'] = isset($row['sort_order']) ? $row['sort_order'] : '';
										$inputdata['is_popular'] = isset($row['is_popular']) ? $row['is_popular'] : '0';
										$inputdata['categories'] = isset($row['category_id']) ? $row['category_id'] : '1';
										$inputdata['created_by'] = isset($row['created_by']) ? $row['created_by'] : 1;
										$inputdata['created_at'] = date('Y-m-d H:i:s');

										$book_id = $this->base_model->insert_operation($inputdata, 'categories');
										if($book_id > 0) {
											$ipdata['book_id'] = $book_id;
											$ipdata['category_id'] = $inputdata['categories'];
											$this->base_model->insert_operation($ipdata, 'book_categories');
										}
										$imported++;
									}
								}
