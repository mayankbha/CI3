...
//Get buyer's discounted price
if (!function_exists('getBuyerDiscountedPrice')) {
	function getBuyerDiscountedPrice($bookId, $userId = '')
	{
		$CI = &get_instance();
		$user = $CI->ion_auth->user()->row();
		if ($userId != '' && is_numeric($userId)) {
		...
		$bookQuery = $CI->db->get_where('seller_selling_books', array('sc_id' => $bookId));
		$bookRow   	=	$bookQuery->row();
		if ($user && $bookRow) {
			$actual_price = $bookRow->actual_price;
			$discount_price = $bookRow->book_price;
			if ((float)$user->admin_discount > 0 && $actual_price > 0) {
				$discount_price_user = ((float)$actual_price - ((float)$actual_price / 100) * (float)$user->admin_discount);
				$discount_price_user = round($discount_price_user, 2);
				$force_discount = $user->force_discount;
				if ($force_discount == '0') {
					if ((float)$discount_price > $discount_price_user) {
						$discount_price = $discount_price_user;
					}
				} else {
					$discount_price = $discount_price_user;
				}
...
if (!function_exists('get_book_img')) {
	function get_book_img($image = "")
	{
		if (!empty($image) && file_exists(URL_UPLOADS_BOOKS_PHYSICAL . $image))
			return URL_UPLOADS_BOOKS . $image;
		else
			return URL_UPLOADS_BOOKS_DEFAULT_IMAGE;
	}
}
if (!function_exists('get_selling_book_img')) {
	function get_selling_book_img($image = "")
	{
		if(!empty($image)) {
			$response = common_s3_function('get', 'file', $image);
		...
		/*if (!empty($image) && file_exists(URL_PUBLIC_UPLOADS . 'book_curriculum_files/' . $image))
			return URL_PUBLIC_UPLOADS2 . 'book_curriculum_files/' . $image;
		else
			return URL_UPLOADS_BOOKS_DEFAULT_IMAGE;*/
	}
}
if (!function_exists('get_seller_img')) {
	function get_seller_img($image = "", $gender = "male")
	{
		if (!empty($image) && file_exists(URL_UPLOADS_PROFILES_PHYSICAL . $image)) {
			...
			if (strcasecmp($gender, 'male') == 0)
				return URL_UPLOADS_PROFILES_SELLER_MALE_DEFAULT_IMAGE;
			else
				return URL_UPLOADS_PROFILES_SELLER_FEMALE_DEFAULT_IMAGE;
		}
	}
}
...
if (!function_exists('get_buyer_img')) {
	function get_buyer_img($image = "", $gender = "male")
	{
		if (!empty($image) && file_exists(URL_UPLOADS_PROFILES_PHYSICAL . $image)) {
			return URL_UPLOADS_PROFILES . $image;
		} else {
			if (strcasecmp($gender, 'male') == 0)
				return URL_UPLOADS_PROFILES_BUYER_MALE_DEFAULT_IMAGE;
			else
				return URL_UPLOADS_PROFILES_BUYER_FEMALE_DEFAULT_IMAGE;
		}
	}
}
...
if (!function_exists('get_user_credits')) {
	function get_user_credits($user_id = "")
	{
		if ($user_id != '' && is_numeric($user_id)) {
			$CI	= &get_instance();
			$CI->db->select("SUM(CASE WHEN action = 'credited' then credits WHEN action = 'debited' then -credits	END ) AS credits");
		
			$CI->db->from('user_credit_transactions');
			$CI->db->where('user_id', $user_id);
		//	$CI->db->where('action', "credited");
			$bookQuery =   $CI->db->get();
			//print_r($CI->db->last_query());die;
			if ($bookQuery->row()->credits) {
				return $bookQuery->row()->credits;
			}
			return 0;
		}
		if (!($user_id > 0)) // || empty($bookQuery->row()->credits))
			return 0;
		//return getUserRec($user_id)->credits;
	}
}
if (!function_exists('get_user_credit_sum')) {
	function get_user_credit_sum($user_id = "",$usertype='buyer')
	{
		$CI = &get_instance();
		if ($user_id == "") {
		...
		if ($user_id != '' && is_numeric($user_id)) {
			...
			if ($bookQuery->row()->credits) {
				...
				if ($usertype == 'seller') {
					$multiplyercredit = $CI->config->item('site_settings')->seller_point_value;
				
				} else {

					if ($CI->ion_auth->is_seller()) {
						$multiplyercredit = $CI->config->item('site_settings')->seller_point_value;
					}elseif ($CI->ion_auth->is_buyer()) {
						$multiplyercredit = $CI->config->item('site_settings')->buyer_point_value;
					}
				}
				$total_return =  $bookQuery->row()->credits * $multiplyercredit;
...
if (!function_exists('is_premium')) {
	function is_premium($user_id = "")
	{
		...
		if ($user_type == "seller")
			$min_credits_for_premium_txt = 'min_credits_for_premium_seller';
		else if ($user_type == "buyer")
			$min_credits_for_premium_txt = 'min_credits_for_premium_buyer';
...
if (!function_exists('is_eligible_to_make_booking')) {
	function is_eligible_to_make_booking($buyer_id = "", $fee = "")
	{
		if (!($buyer_id > 0) || empty($fee))
			return FALSE;
		$buyer_net_credits = get_user_credits($buyer_id);
		if ($buyer_net_credits >= $fee)
			return TRUE;
		else
			return FALSE;
	}
...
if (!function_exists('is_inst_seller')) {
	function is_inst_seller($seller_id = "")
	{
		if (!($seller_id > 0))
			return FALSE;
		$CI	= &get_instance();
		$rs = $CI->db->select('parent_id')->get_where($CI->db->dbprefix('users'), array('id' => $seller_id));
...
if (!function_exists('get_lessons_taught_cnt')) {
	function get_lessons_taught_cnt()
	{
		...
		$enrollments_cnt = $CI->db->query("SELECT sum(if(status = 'closed', 1, 0)) as enrollments_cnt FROM " . TBL_INST_ENROLLED_BUYERS . " ")->row()->enrollments_cnt;
...
if (!function_exists('safe_redirect')) {
	function safe_redirect($url = '', $message = '', $message_type = 'info')
	{
		...
		if (!$CI->ion_auth->logged_in()) {
			$new_url = 'auth/login';
		} elseif ($CI->ion_auth->is_seller()) {
			$new_url = 'seller/index';
		} elseif ($CI->ion_auth->is_buyer()) {
			$new_url = 'buyer/index';
...
function get_safe_url($case = 'subscription')
{
	...
	if ($case == 'subscription') {
		if (!$CI->ion_auth->logged_in()) {
			...
			$new_url = URL_SELLER_SUBSCRIPTIONS;
		} elseif ($CI->ion_auth->is_buyer()) {
			$new_url = URL_BUYER_SUBSCRIPTIONS;
		} elseif ($CI->ion_auth->is_institute()) {
...
if (!function_exists('get_seller_sellingbook_info')) {
	function get_seller_sellingbook_info($sc_id = "")
	{
		$seller_sellingbook_info = array();

		if (!($sc_id > 0))
			return $seller_sellingbook_info;
		$CI	= &get_instance();

		$query = "SELECT sc.*, t.username, t.slug AS seller_slug, t.profile, t.photo, t.gender FROM " . TBL_PREFIX . "seller_selling_books sc INNER JOIN " . TBL_PREFIX . "users t ON t.id=sc.seller_id WHERE sc.sc_id=" . $sc_id . " ";

		$record = $CI->db->query($query)->result();

		if (!empty($record)) {
			$record[0]->sellingbook_curriculum = $CI->db->get_where('seller_selling_books_curriculum', array('sc_id' => $sc_id))->result();

			if(!empty($record[0]->sellingbook_curriculum)) {
				foreach($record[0]->sellingbook_curriculum as $key => $val) {
					$file_name = $val->file_name;

					$response = common_s3_function('get', 'file', $file_name);

					if(!empty($response) && $response['status'] == 'success') {
						$bookCurriculumFileBody = base64_decode($response['message']);

						$f = finfo_open();
						$mime_type = finfo_buffer($f, $bookCurriculumFileBody, FILEINFO_MIME_TYPE);

						$record[0]->sellingbook_curriculum[$key]->s3_file = $response['message'];
						$record[0]->sellingbook_curriculum[$key]->mimetype = $mime_type;
						$record[0]->sellingbook_curriculum[$key]->presignedUrl = $response['presignedUrl'];
					} else {
			...
			$seller_sellingbook_info = $record[0];

			$record[0]->sellingbook_exam = $CI->db->get_where('seller_selling_books_exam', array('sc_id' => $sc_id))->result();
		}
...
function seller_get_config($item)
{
	$CI	= &get_instance();
	$item_val = $CI->config->item($item);
	return (!empty($item_val)) ? $item_val : '';
}
/******Seller Avg. Rating Value ******/
if (!function_exists('sellerAvgRatingValue')) {
	function sellerAvgRatingValue($seller_id = null)
	{
		$rating_value = 0;
		if (is_numeric($seller_id)) {
			$CI = &get_instance();
			$query = "select AVG(tr.rating) as rating_value from " . TBL_PREFIX . "seller_reviews tr WHERE tr.seller_id=" . $seller_id . " AND tr.status='Approved'";
			$record = $CI->db->query($query)->result();
			if (!empty($record))
...
/******Seller no of comments ******/
if (!function_exists('sellerCommentNumber')) {
	function sellerCommentNumber($seller_id = null)
	{
		$no_of_comments = 0;
		if (is_numeric($seller_id)) {
			$CI = &get_instance();
			$query = "select count(tr.id) as no_of_comments from " . TBL_PREFIX . "seller_reviews tr WHERE tr.seller_id=" . $seller_id . " AND tr.status='Approved'";
			$record = $CI->db->query($query)->result();
			if (!empty($record))
...
function update_notification($link)
{
	$CI	= &get_instance();
	$CI->load->database();
	$notifications = $CI->base_model->fetch_records_from("notifications", array(
		'page_link' => $link,
		'admin_read' => 0
	));
...
//$method - get/get_and_save/set/del
//$type - single file (all type of files like jpg, png, pdf, txt etc.) or zip
//$file_name - file name to be upload
//$file_path - local file path of the file
//$base64_encode - the file to be written is base64_encoded or normal file
if (!function_exists('common_s3_function')) {
	function common_s3_function($method = 'get', $type = 'file', $file_name = '', $file_path = '', $base64_encode = true) {
		$s3Client = new Aws\S3\S3Client([
						'version' => S3_VERSION,
						'region'  => S3_REGION,
						'signatureVersion' => S3_SIGNATUREVERSION,
						'credentials' => [
							'key' => S3_SECRET_KEY,
							'secret' => S3_ACCESSS_KEY
							//'key' => 'AKIARBG2V74LHONZXJLD',
							//'secret' => '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'
						]
					]);

		$bucket_name = S3_BUCKET_NAME;
		$book_s3_upload_path = S3_BOOKS_FILE_UPLOAD_PATH;

		$key = $book_s3_upload_path.$file_name;
		...
		if($method == 'get') {
			if($file_name != '') {
				if($type == 'file') {
					try {
						$getFile = $s3Client->getObject([
										'Bucket' => $bucket_name,
										'Key' => $key
									]);

						$fileBody = $getFile->get('Body');
						...
						$response['message'] = base64_encode($fileBody);
			...
			if($file_name != '') {
				try {
					...
					//file_put_contents($file_path.$file_name, $fileBody);

					$response['status'] = 'success';
					$response['ContentType'] = $getFile->get('ContentType');
					$response['message'] = $fileBody;
				} catch (Aws\S3\Exception\S3Exception $e) {
					$response['status'] = 'error';
					$response['message'] = $e->getMessage();
				}
			...
			if($file_name != '') {
				if($type == 'file') {
					...
					try {
						...
						$temp_path = $file_path.$file_name;

						$size = filesize($temp_path);
						$sha256 = hash_file("sha256", $temp_path);
						...
						try {
							$result = $s3Client->putObject([
										...
										//'Body' => fopen($file_path, 'r'),
										'ContentType' => 'applicationimage/zip',
										'ContentLength' => $size,
										//'ACL'    => 'public-read-write',
										'ContentSHA256' => $sha256
									]);
						...
						/*$zip = new ZipArchive;
						$res = $zip->open($temp_path, ZIPARCHIVE::CREATE | ZIPARCHIVE::OVERWRITE);
