...
class MY_Controller extends CI_Controller
{
	...
	function __construct()
	{
		...
		if($this->is_demo) {
			...
			$allowed_urls = array('auth/login', 'home/search_seller', 'home/search_institute', 'home/search_buyer_leads', 'institute/enrolled_buyers', 'seller/my_batches');

			$restricted_urls = array('institute/approve_batch_buyers', 'institute/send_credits_conversion_request', 'seller/initiate_batch_session', 'seller/complete_batch_session');
			...
			if($this->input->post()) {
				...
				if(!($this->input->is_ajax_request() || in_array($redirect_path, $allowed_urls))) {
					...
					if($this->data['current_method'] == "book_seller")
				...
				if($this->data['current_method'] == "approve_batch_buyers")
					redirect(URL_INSTITUTE_ENROLLED_BUYERS);
...
function generateRatingStar($id, $average = 3)
{
	...
	public function grocery_output($output = null)
	{
		...
		$this->load->view('template/site/seller-template-grocery',$output);
		elseif($this->ion_auth->is_buyer())
		$this->load->view('template/site/buyer-template-grocery',$output);
	...
	{
		if (!$this->ion_auth->logged_in() || !($this->ion_auth->is_seller() || (($this->data['current_method'] == "list_selling_books" || $this->data['current_method'] == "view_selling_book_curriculum") && $this->ion_auth->is_admin()))) {
			$this->prepare_flashmessage(get_languageword('You dont have permission to access this page'), 1);
			redirect('auth/login', 'refresh');
		}
		else if(isset($this->config->item('site_settings')->need_admin_for_seller) && $this->config->item('site_settings')->need_admin_for_seller == 'Yes')
		{
			if(!is_uploaded_certificates($this->ion_auth->get_user_id())) {
				...
				if($this->data['current_controller'] == 'seller')
				{
					if($this->data['current_method'] != 'certificates')
						redirect('seller/certificates', 'refresh');
				}
			}
