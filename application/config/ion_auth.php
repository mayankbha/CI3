...
if ($hostname=='' || $username=='' || $database=='') {
	redirect(SITEURL.'install');
		...
		/****** Get One Signal Push Notifications Settings ******/
		$oresults = $CI->db->query('SELECT * FROM '.$CI->db->dbprefix(TBL_SETTINGS_FIELDS).' sf INNER JOIN '.$CI->db->dbprefix(TBL_SETTINGS_TYPES).' types on sf.type_id = types.type_id WHERE type_slug="PUSH_NOTIFICATION_SETTINGS"')->result();
		
		$one_signal_settings = array();
		foreach($oresults as $r) {
			$one_signal_settings[strtolower($r->field_key)] =  $r->field_output_value;
		}



		$CI->config->set_item('one_signal_settings', (object)$one_signal_settings);
...
$config['default_group']             = 'buyer';           // Default group, use name
$config['member_group']              = 'buyer';           // Default group, use name
$config['admin_group']               = 'admin';             // Default administrators group, use name
$config['seller_group']           	 =	'seller';          // seller User Group
