...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function db_insert_validation()
	{
		...
		if($CI->is_demo) {
			...
			$validation_result->error_message = get_languageword("CRUD_opeartions_disabled_in_Demo_version");
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function delete_layout($delete_result = true)
	{
		...
		if($delete_result === false)
		{
			...
			if($CI->is_demo) {
				...
				$error_message = '<p>'.get_languageword('CRUD_opeartions_disabled_in_Demo_version').'</p>';
			}
	...
	protected function get_enum_input($field_info,$value)
	{
		...
		{
			$selected = !is_null($value) && $value == $option ? "selected='selected'" : '';
...
class Grocery_CRUD extends grocery_CRUD_States
{
	 ...
	 */
	public function change_field_type($field , $type, $extras = null, $default_value = null)
	{
		$field_type = (object)array('type' => $type);

		$field_type->extras = $extras;
		if(!empty($default_value))
		$field_type->default_value = $default_value;
	...
	{
		return $this->change_field_type($field , $type, $extras, $default_value);
	}
